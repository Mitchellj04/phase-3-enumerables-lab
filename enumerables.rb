...
def spiciest_foods(spicy_foods)
  spicy_foods.select do |heatlvl|
    heatlvl[:heat_level] > 5
  end
end
...
def print_spiciest_foods(spicy_foods)
  spiciest_foods(spicy_foods).each do |foods|
    puts "#{foods[:name]} (#{foods[:cuisine]}) | Heat Level: #{"🌶" * foods[:heat_level]}"
  end
end
...
def average_heat_level(spicy_foods)
  total = 0
  spicy_foods.each do |count|
    total += count[:heat_level]
  end
